...
export function OrganizerSidebar({ sidebarOpen, setSidebarOpen, isOrganizer }: OrganizerSidebarProps) {
  ...
  // 2. Use the hook to get null and connection status
  const { publicKey, connected, disconnect } = useWallet();
  const isDisconnecting = false; // Wallet adapter doesn't expose loading state
  // connected is already available from useWallet

  // 3. The old useState and useEffect for null are no longer needed and have been removed.
          ...
          <div className="mt-auto px-6 py-4 border-t border-gray-800">
            <div className="flex items-center gap-3">
              ...
              <div className="flex flex-col overflow-hidden">
                <span className="font-bold text-white text-sm leading-tight truncate">User</span>
                <span className="text-gray-400 text-xs leading-tight truncate">Wallet Connected</span>
