...
import { CypherpunkSecurity } from "../../../components/CypherpunkSecurity";
...
export function MergedDashboard() {
  ...
  const organizerWallet = publicKey?.toString() || "";
  const isConnected = connected && !!organizerWallet;
  ...
  useEffect(() => {
    const initAuth = async () => {
      try {
        ...
        // Don't block the UI if Firebase auth fails
        // Allow the user to proceed without Firebase features
        setIsAuthReady(true);
        setDataLoading(false);
      }
    };

    // Set a timeout for Firebase auth
    const authTimeout = setTimeout(() => {
      console.warn("Firebase auth timed out, proceeding without it");
      setIsAuthReady(true);
      setDataLoading(false);
    }, 3000);
  ...
  useEffect(() => {
    ...
    const timeoutId = setTimeout(() => {
      console.warn("Data fetch timed out, showing empty state");
      setDataLoading(false);
    }, 8000);
